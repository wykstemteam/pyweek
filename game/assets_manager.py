...
class AssetsManager:
    ...
    def init_images(self) -> None:
        for fn in os.listdir(os.path.join('assets', 'images')):
            ...
            self.images[base_fn] = pygame.image.load(
                os.path.join('assets', 'images', fn))

    def scale_images(self) -> None:
        # background
        self.images['road'] = pygame.transform.scale(
            self.images['road'], (SCREEN_WIDTH,
                                  SCREEN_HEIGHT - BUILDING_HEIGHT)
        )
        ...
        self.images['arrow'] = pygame.transform.scale(
            self.images['arrow'], (80, 100))
        for i in range(5):
            self.images[f'HP{i}'] = pygame.transform.scale(
                self.images[f'HP{i}'], (400, 100))
        for i in range(5):
            self.images[f'HP{i}_shop'] = pygame.transform.scale(
                self.images[f'HP{i}'], (240, 60))
        ...
        self.images['bomber_bullet'] = pygame.transform.scale(
            self.images['bomber_bullet'], (BOMBER_BULLET_WIDTH,
        ...
        self.images['missile_for_aircraft'] = pygame.transform.scale(
            self.images['missile_for_aircraft'], (
                MISSILE_AIRCRAFT_WIDTH, LASER_HEIGHT)
        )
        ...
        self.images['spaceship_bullet'] = pygame.transform.scale(
            self.images['spaceship_bullet'], (SPACESHIP_BULLET_WIDTH,
        ...
        for i in range(1, 4):
            self.images[f'shield{i}'] = pygame.transform.scale(
                ...
                (PLAYER_WIDTH + SHIELD_WIDTH_OFFSET,
                 PLAYER_HEIGHT + SHIELD_HEIGHT_OFFSET)
            )
        for item_name in ('item_blank', 'item_healpotion', 'item_shield', 'item_star', 'item_clock', 'item_missile', 'item_earthquake'):
    ...
    def init_sounds(self) -> None:
        for fn in os.listdir(os.path.join('assets', 'sounds')):
            base_fn = os.path.splitext(fn)[0]
            self.sounds[base_fn] = pygame.mixer.Sound(
                os.path.join('assets', 'sounds', fn))
